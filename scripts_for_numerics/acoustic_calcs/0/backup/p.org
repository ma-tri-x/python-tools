...
//         psi           psim;
   ...
   of lInf, the more reflective the boundary tends to be.*/
     ...
     {
        type            groovyBC;
        variables       "amp=300000.0;omega=2*pi*25120.0;k=2.*pi/0.000808/4.;";
        valueExpression  "101315.0 + amp * sin(omega*time())";  //"amp*sin(omega*time())*normal()";
        value           uniform 101315.0;
     }
//         valueExpression  "-amp*omega*sin(omega*time()) + 101315.0";  //"amp*sin(omega*time())*normal()";
    //*cos(k*pos().x)
}
